<template>
  <div class="music flex-col">
    ...
    <div class="music-bar" :class="{ disable: !musicReady || !currentMusic.id }">
      ...
      <!-- 分离功能 -->
      <button type="button" class="select_" @click="openModel()">AI模型</button>
...
import { PLAY_MODE, PLAY_SEP, MMPLAYER_CONFIG } from '@/config'
...
export default {
  ...
  methods: {
    ...
    async modelChoice() {
      try {
        const response = await axios.get('http://localhost:8080/#/music/playlist')
    ...
    openModel() {
      ...
      this.$router.push(`/music/modelSep/${this.currentMusic.id}`)
    },
    // 修改音量大小
